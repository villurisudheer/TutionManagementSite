...
(async()=>{try{
 await wait();
 const h=await req('/api/health');if(!h.ok||h.version!=='5.0.0')throw new Error('health/version failed');
 ...
 const adminPage=await fetch(base+'/admin');const adminHtml=await adminPage.text();if(!adminHtml.includes('Private Tuition Manager')||!adminHtml.includes('uiTheme')||!adminHtml.includes('mobileNav'))throw new Error('V5 adaptive admin page not served');
