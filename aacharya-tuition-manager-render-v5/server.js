...
function makeGuideSheet() {
  const rows=[
    ['AACHARYA LEARNING ACADEMY — V5 MASTER WORKBOOK','','','',''],
    ['Comprehensive operational export • generated automatically from the live tuition-manager database','','','',''],
    ['Generated',new Date().toLocaleString('en-IN',{timeZone:'Asia/Kolkata'}),'Workbook Version','5.0','Sheets',V5_SHEET_COUNT],
    [],
...
function makeDashboardSheet(ctx) {
  ...
  r=push(['V5 comprehensive academy snapshot • automatically regenerated from live data',...Array(9).fill('')],'subtitle');merges.push(`A${r}:J${r}`);heights[r]=28;
...
function rebuildWorkbook() {
  ...
  appendReport(wb,{name:'Data Dictionary',title:'DATA DICTIONARY',subtitle:'Definitions for important calculated and operational fields in the V5 workbook.',columns:[{key:'sheet',label:'Sheet',width:24},{key:'field',label:'Field',width:28},{key:'definition',label:'Definition / Calculation',width:86}],data:dict,tabColor:'7F7F7F'});
...
app.get('/api/health', (req, res) => res.json({
  ok: true,
  service: 'Aacharya Tuition Manager v5',
  version: '5.0.0',
  dataDir: DATA_DIR,
...
app.get('/api/excel/status',authRequired,(req,res)=>{let stat=null;try{stat=fs.statSync(EXCEL_PATH);}catch{}res.json({ok:true,path:EXCEL_PATH,exists:!!stat,size:stat?.size||0,updatedAt:stat?.mtime?.toISOString()||null,version:'5.0.0',sheetCount:V5_SHEET_COUNT});});
