...
(async()=>{try{
 await wait();
 const h=await req('/api/health');if(!h.ok||h.version!=='5.2.0')throw new Error('health/version failed');
 ...
 const adminPage=await fetch(base+'/admin');const adminHtml=await adminPage.text();if(!adminHtml.includes('Private Tuition Manager')||!adminHtml.includes('uiTheme')||!adminHtml.includes('uiLightPrimary')||!adminHtml.includes('uiDarkSecondary')||!adminHtml.includes('mobileNav'))throw new Error('V5.2 adaptive admin page not served');
 ...
 const file=path.join(dir,'Aacharya_Tuition_Master_v5.xlsx');if(!fs.existsSync(file))throw new Error('master xlsx missing');
 const raw=fs.readFileSync(file).toString('utf8');const expected=['Workbook Guide','Executive Dashboard','Student Master','Student 360','Parent Directory','Academic Profiles','Class Register','Class Hours Summary','Monthly Class Hours','Attendance Register','Attendance Summary','Monthly Attendance','Fee Register','Payments Ledger','Student Finance','Outstanding Dues','Monthly Collections','Payment Methods','Test Register','Performance Summary','Subject Performance','Intake Submissions','Admissions Pipeline','Data Quality','Audit Log','System Settings','Data Dictionary'];for(const n of expected)if(!raw.includes(`name=\"${n}\"`))throw new Error('missing sheet '+n);if(!raw.includes('Smoke Parent')||!raw.includes('Test Admin')||!raw.includes('Paid By')||!raw.includes('Received By')||!raw.includes('Student 360')||!raw.includes('Data Quality'))throw new Error('V5.2 comprehensive Excel details missing');
 await req('/api/auth/logout',{method:'POST',body:'{}'});
 console.log('SMOKE TEST PASSED: login, public intake, students, classes, attendance, fees, payer/receiver payments, tests, dashboard, V5.2 27-sheet styled Excel workbook, logout.');
