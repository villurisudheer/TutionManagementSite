...
const EXCEL_PATH = path.join(DATA_DIR, 'Aacharya_Tuition_Master_v4.xlsx');
...
const V4_SHEET_COUNT=V4_SHEET_CATALOG.length;

function makeGuideSheet() {
  const rows=[
    ['AACHARYA LEARNING ACADEMY — V4 MASTER WORKBOOK','','','',''],
    ['Comprehensive operational export • generated automatically from the live tuition-manager database','','','',''],
    ['Generated',new Date().toLocaleString('en-IN',{timeZone:'Asia/Kolkata'}),'Workbook Version','4.0','Sheets',V4_SHEET_COUNT],
    [],
    ...
    ...V4_SHEET_CATALOG.map(([name,desc],i)=>[name,desc,i<2?'Dashboard / Guide':i>=24?'System / Reference':'Operational / Analytics','Automatic after data changes','Use Excel filters on detailed registers.']),
    [],
    ['HOW TO USE','','','',''],
  ...
  ws['!merges']=['A1:F1','A2:F2','A5:F5','A36:F36'];
  ws['!rowStyles']={1:'title',2:'subtitle',3:'meta',5:'note',7:'header',36:'section'};
  ws['!rowHeights']={1:32,2:28,5:38,7:30,36:26};
  ws['!freeze']={rows:7};ws['!autofilter']=`A7:E${7+V4_SHEET_CATALOG.length}`;ws['!dataStartRow']=8;ws['!alternateRows']=true;
...
function makeDashboardSheet(ctx) {
  ...
  r=push(['V4 comprehensive academy snapshot • automatically regenerated from live data',...Array(9).fill('')],'subtitle');merges.push(`A${r}:J${r}`);heights[r]=28;
  push(['Generated',new Date().toLocaleString('en-IN',{timeZone:'Asia/Kolkata'}),'Academic Year',one("SELECT value FROM settings WHERE key='academic_year'")?.value||'','Workbook Sheets',V4_SHEET_COUNT,'','','',''],'meta');blank();
...
function rebuildWorkbook() {
  ...
  const studentRaw=rows(`SELECT id,student_code,full_name,dob,gender,school,board,grade,stream,joining_date,phone,email,address,parent_name,parent_phone,parent_email,subjects,fee_plan_amount,status,notes,intake_submission_id,created_at,updated_at FROM students ORDER BY full_name`);
  ...
  const studentFinance=rows(`SELECT s.id,s.student_code,s.full_name student,s.status,s.grade,s.subjects,s.fee_plan_amount current_fee_plan,
    ...
    FROM students s ORDER BY s.full_name`).map(x=>({...x,pending_balance:Math.max(0,safeNumber(x.fees_billed)-safeNumber(x.total_paid)),credit_balance:Math.max(0,safeNumber(x.total_paid)-safeNumber(x.fees_billed))}));
  ...
  appendReport(wb,{name:'Data Dictionary',title:'DATA DICTIONARY',subtitle:'Definitions for important calculated and operational fields in the V4 workbook.',columns:[{key:'sheet',label:'Sheet',width:24},{key:'field',label:'Field',width:28},{key:'definition',label:'Definition / Calculation',width:86}],data:dict,tabColor:'7F7F7F'});
...
app.get('/api/health', (req, res) => res.json({
  ok: true,
  service: 'Aacharya Tuition Manager v4',
  version: '4.0.0',
  dataDir: DATA_DIR,
  excelFile: path.basename(EXCEL_PATH),
  excelSheets: V4_SHEET_COUNT,
...
app.get('/api/excel/status',authRequired,(req,res)=>{let stat=null;try{stat=fs.statSync(EXCEL_PATH);}catch{}res.json({ok:true,path:EXCEL_PATH,exists:!!stat,size:stat?.size||0,updatedAt:stat?.mtime?.toISOString()||null,version:'4.0.0',sheetCount:V4_SHEET_COUNT});});
...
app.get('/api/export/xlsx',authRequired,(req,res)=>{try{rebuildWorkbook();res.download(EXCEL_PATH,'Aacharya_Tuition_Master_v4.xlsx');}catch(e){console.error(e);res.status(500).json({ok:false,message:'Could not create Excel workbook.'});}});
